...
### Added
- **"Invert X axis" / "Invert Y axis" in the stitch dialog, for the MicroManager method.** MMStack sidecars carry absolute stage coordinates, and whether a rising stage coordinate moves right and down in the camera image depends on how the stage is wired and the camera mounted. The strategy assumes rising stage equals rising pixel; where an axis runs the other way every tile landed in its mirrored slot, and the dialog offered no way to say so -- the `flipStitchingX` / `flipStitchingY` flags existed, but only QPSC ever set them, so stitching the same folder from this extension's own dialog produced a mosaic in which no seam matched. The failure reads as plausible, because the mosaic is still the right size and the overlap still measures correctly; with registration on it is unmistakable, since every edge is rejected ("no edge survived the confidence gates"). The checkboxes appear only for the MicroManager method, are remembered between stitches, and the axes actually used are recorded in the `.stitch-info.txt` written beside the output. Measured on a 3x3 four-channel acquisition from an inverted-stage scope: with the axes wrong, overlap-band correlation ran -0.04 to +0.12 and 0 of 12 seams were accepted; with them right, 0.52 to 0.74 and 12 of 12.
